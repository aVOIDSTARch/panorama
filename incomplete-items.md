...
Password placeholder only	Need webauthn-rs crate, FIDO2 registration ceremony, credential storage in Datastore, session binding to hardware key
### Telnyx Ed25519 verification	Checks
signature presence, not validity
### Implement actual Ed25519
verify against TELNYX_PUBLIC_KEY in inbound.rs
### TOTP for owner commands
TODO comment in identity.rs	TOTP token extraction from SMS body for Owner-level auth
### Recognized sender list
TODO in identity.rs	Datastore-backed list of previously-seen senders
...
### CI/CD
None	GitHub Actions for cargo test, npm test, pytest
### Infisical	Client
complete, needs deployment	Real Infisical instance URL + token in .env
